# Student-Data-Dashboard-
A modern Student Data Dashboard built with Python and CustomTkinter to manage course-wise student records. Features include add, update, delete, search, and real-time statistics. Uses JSON for data storage and provides a clean, user-friendly GUI.

# Student Data Dashboard

A Python-based desktop application built with CustomTkinter for managing student records in a structured and user-friendly way. The system supports multiple courses and provides an intuitive interface for handling student data efficiently.

## Features

* Course-wise student management
* Add, update, delete, and search student records
* Real-time dashboard statistics
* Recently added students tracking
* JSON-based data storage (no database required)
* Clean and responsive GUI

## Tech Stack

* Python
* CustomTkinter
* JSON
* Pillow

## Project Structure

```
Student-Data-Dashboard/
│
├── student_dashboard.py
├── student_data.json
├── logo.png
└── README.md
```

## Installation

1. Clone the repository:

```
git clone https://github.com/your-username/Student-Data-Dashboard.git
cd Student-Data-Dashboard
```

2. Install dependencies:

```
pip install customtkinter pillow
```

3. Run the application:

```
python student_dashboard.py
```

## Usage

Run the application and use the interface to manage courses and student records. Data is automatically stored in a JSON file.

## Future Improvements

* Database integration
* Data export (CSV/Excel)
* Authentication system
* Web-based version

## Author

Ravi Roshan
https://github.com/ravi2002roshan
